...
main PROC
	...
	call	Clrscr
		...
		mov esi,OFFSET	intArray				;offset by which I will increment place in array

	FillArray:									;this loop will asist with user inputing data into the array

		call	ReadInt
		mov	[esi],	eax							;put data in array in place of offset

		add	esi,	4							;increment offset place

	Loop FillArray

		mov	esi,OFFSET	intArray				;offset esi to intarray
		mov edx,	[esi]						;copy first position of intarray into edx
	...
	AddArray:									;this loop adds the contents of the array to eachother
		...
		add	esi,	4							;increment esi by 4 which is qword size
		add	eax,	[esi]						;add whatever is in eax and esi position of array
	...
	displayArray:								;this loop displays the contents of the array
		...
		mov	eax,	[esi]						;put array value into eax
		call	WriteInt
		add	esi,	4							;increment offset place
		call	Crlf							;new line
